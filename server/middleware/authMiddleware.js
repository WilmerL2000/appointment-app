 ...
 * assigns the decoded token's ID to the `req.vet` property for use in subsequent middleware functions
 * or routes.
 ...
 * token's ID to the `req.vet` property and then calls `next()`. If the `authorization` header is
 * missing
 */
const checkToken = async (req, res, next) => {
  ...
  if (authorization && authorization.startsWith('Bearer ')) {
    try {
      ...
      /* `req.vet` start sesion in espress is assigning the result of a MongoDB query to the `vet` property of the `req`
